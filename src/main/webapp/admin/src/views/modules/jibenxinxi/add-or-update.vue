...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.leibieOptions = "担子菌亚门,子囊菌亚门".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `jibenxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.xiangxijieshao = this.ruleForm.xiangxijieshao.replace(reg,'../../../ssmh79by/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jibenxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jibenxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
